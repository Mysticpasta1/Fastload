...
import com.mystic.quickboot.init.QuickBoot;
import com.mystic.quickboot.util.Bound;
...
import static com.mystic.quickboot.config.QBMath.ifDebugEnabled;
...
public class QBConfig {
    ...
    static {
        ...
        path = FMLPaths.CONFIGDIR.get().resolve(QuickBoot.NAMESPACE.toLowerCase() + ".properties");
    ...
    private static void logWarn(String key) {
        QuickBoot.LOGGER.warn("Failed to parse variable '" + key + "' in " + QuickBoot.NAMESPACE + "'s config, " +
    ...
    public static void writeToDisk() {
        try (OutputStream out = Files.newOutputStream(path, StandardOpenOption.TRUNCATE_EXISTING, StandardOpenOption.WRITE, StandardOpenOption.CREATE)) {
            properties.store(out,  QuickBoot.NAMESPACE +  " Configuration File");
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
        try (BufferedWriter comment = Files.newBufferedWriter(path, StandardOpenOption.APPEND, StandardOpenOption.WRITE, StandardOpenOption.CREATE)) {
            comment.write("\n# Definitions");
            comment.write("\n# " + writable(DEBUG_KEY) + " = debug (logWarn) all things happening in quick boot to aid in " +
                    "diagnosing issues.");
            ...
            comment.write("\n#");
            comment.write("\n# " + writable(INSTANT_LOAD_KEY) + " = should quick boot just nuke the downloading terrain" +
                    " screen? This boolean is for impatient people who think quick boot's loading screen isn't " +
                    "necessary.");
            comment.write("\n# Enabled = true, Disabled = false");
            comment.write("\n#");
    ...
    public static void storeProperty(String key, String value) {
        ifDebugEnabled(() -> QuickBoot.LOGGER.info(key + ":" + value));
